...
| **`serviceData`**      | <code>ServiceDataFilter[]</code>              | Filter devices by advertised service data payloads. Each filter matches on a specific service UUID and optional masked prefix.                                                                                                                            |
...
| Prop         | Type                                              | Description                                                                                                                                                                                     |
| ------------ | ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`service`** | <code>string</code>                               | Service UUID to match against the advertisement payload.                                                                                                                                        |
| **`data`**    | <code><a href="#uint8array">Uint8Array</a></code> | Prefix to match in the service data field. Must contain at least one byte.                                                                                                                      |
| **`mask`**    | <code><a href="#uint8array">Uint8Array</a></code> | Optional mask to apply when matching the service data prefix. The `mask` must have the same length as `data`. On **Android** and **iOS** it enables masked comparisons. Ignored on unsupported platforms. |
